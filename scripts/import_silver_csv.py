#!/usr/bin/env python3
"""
Import Silver futures price data from a CSV or Numbers file into
data/silver_prices.sqlite.
...
Usage:
  python3 scripts/import_silver_csv.py path/to/silver_data.csv
  python3 scripts/import_silver_csv.py path/to/silver_data.numbers
"""
...
SQLITE_PATH = DATA_DIR / "silver_prices.sqlite"
...
COLUMN_MAP = {
    ...
    "vol":                           "volume",
    "volume":                        "volume",
    "prev. day open interest":       "volume",  # sometimes only OI is available
    # % change
    "change %":                      "pct_change",
    "change%":                       "pct_change",
    "chg%":                          "pct_change",
...
def ensure_table(conn: sqlite3.Connection):
    ...
    """)
    conn.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_silver_date ON {TABLE_NAME}(date)")
    conn.commit()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/import_silver_csv.py <path/to/file.csv|.numbers>")
        sys.exit(1)

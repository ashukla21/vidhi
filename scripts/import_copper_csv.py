#!/usr/bin/env python3
"""
Import Copper Futures price data from a CSV or Numbers file into
data/copper_prices.sqlite.
...
Handles common source formats:
  ...
  Quandl/COMEX:   Date, Open, High, Low, Settle, Volume, Prev. Day Open Interest
  LME:            Date, Cash Buyer, Cash Seller, 3-Month Buyer, 3-Month Seller
  Simple:         Date, Price  (or Date, Close)

Usage:
  python3 scripts/import_copper_csv.py path/to/copper_data.csv
  python3 scripts/import_copper_csv.py path/to/copper_data.numbers
"""
...
SQLITE_PATH = DATA_DIR / "copper_prices.sqlite"
TABLE_NAME  = "copper_prices"
...
COLUMN_MAP = {
    ...
    "spot price":                             "close",
    "copper price":                           "close",
    "copper spot price":                      "close",
    "copper":                                 "close",
    "hg price":                               "close",
    "hg1 price":                              "close",
    "copper futures":                         "close",
    "copper futures price":                   "close",
    "comex copper":                           "close",
    "lme copper":                             "close",
    "cash buyer":                             "close",   # LME cash buyer = spot
    "cash seller":                            "close",
    "value":                                  "close",
    "adj close":                              "close",
    ...
    "open":                                   "open",
    "open price":                             "open",
    "3-month buyer":                          "open",   # LME 3-month as proxy
    # High / Low
    "high":                                   "high",
    "low":                                    "low",
    # Volume
...
def ensure_table(conn: sqlite3.Connection):
    ...
    """)
    conn.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_copper_date ON {TABLE_NAME}(date)")
    conn.commit()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/import_copper_csv.py <path/to/file.csv|.numbers>")
        sys.exit(1)

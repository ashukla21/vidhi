#!/usr/bin/env python3
"""
Import WTI Crude Oil price data from a CSV or Numbers file into
data/crude_prices.sqlite.
...
Handles common source formats:
  ...
  Quandl/EIA:     Date, Value  (or Date, Open, High, Low, Close, Volume)
  EIA:            Date, WTI Crude Oil Prices Dollars per Barrel
  Simple:         Date, Price  (or Date, Close)

Multiple files can be uploaded one at a time — rows are upserted so
all date ranges merge into a single DB.

Usage:
  python3 scripts/import_crude_csv.py path/to/crude_data.csv
  python3 scripts/import_crude_csv.py path/to/crude_data.numbers
"""
...
SQLITE_PATH = DATA_DIR / "crude_prices.sqlite"
TABLE_NAME  = "crude_prices"
...
COLUMN_MAP = {
    ...
    # EIA long-form headers
    "wti crude oil prices dollars per barrel":                       "close",
    "cushing ok wti spot price fob dollars per barrel":             "close",
    "cushing, ok wti spot price fob (dollars per barrel)":          "close",
    "u.s. wti crude oil spot price (dollars per barrel)":           "close",
    "west texas intermediate (wti) crude oil price ($/bbl)":        "close",
    # Open
    "open":                                                          "open",
    "open price":                                                    "open",
    # High / Low
...
def ensure_table(conn: sqlite3.Connection):
    ...
    """)
    conn.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_crude_date ON {TABLE_NAME}(date)")
    conn.commit()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/import_crude_csv.py <path/to/file.csv|.numbers>")
        sys.exit(1)

...
const SQLITE_PATH = path.join(process.cwd(), "data", "ng_prices.sqlite");
...
export function invalidateNgDb(): void {
  try { db?.close(); } catch { /* ignore */ }
  db = null;
  dbPath = null;
}

export function isNgDataReady(): boolean {
  return fs.existsSync(SQLITE_PATH);
}

function querySync(sql: string, params: unknown[] = []): Record<string, unknown>[] {
  if (!isNgDataReady()) {
    throw new Error("Natural Gas data not loaded. Upload a CSV via the sidebar.");
  }
...
// `close` holds the spot price ($/MMBtu for Henry Hub)

export function getNaturalGasPrices(params: {
  startDate: string;
  endDate: string;
  limit?: number;
...
export function getNgDateRange(): { earliest: string; latest: string } | null {
  if (!isNgDataReady()) return null;
  try {
    const row = querySync(
      "SELECT MIN(date) as earliest, MAX(date) as latest FROM ng_prices"
    )[0];

...
const SQLITE_PATH = path.join(process.cwd(), "data", "silver_prices.sqlite");
...
export function invalidateSilverDb(): void {
  try { db?.close(); } catch { /* ignore */ }
  db = null;
  dbPath = null;
}

export function isSilverDataReady(): boolean {
  return fs.existsSync(SQLITE_PATH);
}

function querySync(sql: string, params: unknown[] = []): Record<string, unknown>[] {
  if (!isSilverDataReady()) {
    throw new Error("Silver price data not loaded. Upload a CSV via the sidebar.");
  }
...
export function getSilverPrices(params: {
  startDate: string;
  endDate: string;
  limit?: number;
...
export function getSilverDateRange(): { earliest: string; latest: string } | null {
  if (!isSilverDataReady()) return null;
  try {
    const row = querySync(
      "SELECT MIN(date) as earliest, MAX(date) as latest FROM silver_prices"
    )[0];

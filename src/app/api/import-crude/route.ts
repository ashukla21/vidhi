...
import os from "os";
import { invalidateCrudeDb } from "@/lib/crude-db";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const tmpFile = path.join(os.tmpdir(), `crude_import_${Date.now()}${ext}`);
    const bytes = await file.arrayBuffer();
    await writeFile(tmpFile, Buffer.from(bytes));

    const scriptPath = path.join(process.cwd(), "scripts", "import_crude_csv.py");

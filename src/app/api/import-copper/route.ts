...
import os from "os";
import { invalidateCopperDb } from "@/lib/copper-db";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const tmpFile = path.join(os.tmpdir(), `copper_import_${Date.now()}${ext}`);
    const bytes = await file.arrayBuffer();
    await writeFile(tmpFile, Buffer.from(bytes));

    const scriptPath = path.join(process.cwd(), "scripts", "import_copper_csv.py");

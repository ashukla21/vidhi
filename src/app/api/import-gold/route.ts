...
import os from "os";
import { invalidateGoldDb } from "@/lib/gold-db";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const tmpFile = path.join(os.tmpdir(), `gold_import_${Date.now()}${ext}`);
    const bytes = await file.arrayBuffer();
    await writeFile(tmpFile, Buffer.from(bytes));

    const scriptPath = path.join(process.cwd(), "scripts", "import_gold_csv.py");

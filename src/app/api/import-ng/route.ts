...
import os from "os";
import { invalidateNgDb } from "@/lib/ng-db";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const tmpFile = path.join(os.tmpdir(), `ng_import_${Date.now()}${ext}`);
    const bytes = await file.arrayBuffer();
    await writeFile(tmpFile, Buffer.from(bytes));

    const scriptPath = path.join(process.cwd(), "scripts", "import_ng_csv.py");

...
export async function POST(req: NextRequest) {
  try {
    const { ticker, company_name, ipo_date, ipo_time, ipo_timezone, ipo_city, ipo_state, ipo_country } = await req.json();
    if (!ticker || typeof ticker !== "string") {
      return Response.json({ error: "ticker is required" }, { status: 400 });
    }
    createStock({ ticker, company_name, ipo_date, ipo_time, ipo_timezone, ipo_city, ipo_state, ipo_country });
